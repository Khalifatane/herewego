(function () {
  ...
  function addCartItem(item) {
    ...
    const existing = currentCart.find(function (entry) {
      return entry.product_id === normalizedItem.product_id &&
    ...
    const nextCart = existing
      ? currentCart.map(function (entry) {
          return entry === existing
            ? normalizeCartItem({
                ...entry,
                quantity: Number(entry.quantity || 0) + Number(normalizedItem.quantity || 1),
              }, 0)
            : entry;
        })
    ...
    window.dispatchEvent(new CustomEvent("cart:update", {
      detail: {
        cart: nextCart,
        count: getFallbackCartCount(nextCart),
      },
    }));
  ...
  function isAddToCartLabel(text) {
    return /(?:^|\b)(add to cart|ajouter au panier|add to basket|ajouter au chariot)(?:\b|$)/.test(normalizeActionLabel(text));
  }
  ...
  function bindAddToCartButtons(root) {
    ...
    scope.querySelectorAll('button, a, [data-commerce-action="add-to-cart"]').forEach(function (element) {
      ...
      if (element.matches("[data-favorite-toggle]")) return;

      const labelNode = element.querySelector(".rfrdb") || element;
      const labelText = labelNode ? labelNode.textContent || "" : element.textContent || "";
      if (!isAddToCartLabel(labelText) && element.dataset.commerceAction !== "add-to-cart") return;

      const originalText = (labelText || "").trim() || "Add to cart";
      ...
      element.addEventListener("click", function (event) {
        ...
        const nextCart = addCartItem(item);
